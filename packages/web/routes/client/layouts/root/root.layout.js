...
export default function defaultRootLayout ({
  ...
  // Build head content as string to handle void elements properly
  const headContent = `
    <meta charset="utf-8">
    ...
    ${image ? `<meta name="twitter:image" content="${image.startsWith('/') ? `${baseUrl}${image}` : `${resolvedURL}${image}`}">` : ''}
    <meta name="twitter:site" content="${siteTwitter}">
    <meta name="twitter:card" content="${image ? 'summary_large_image' : 'summary'}">
    <meta name="twitter:title" content="${title ?? siteName}">
    <meta name="twitter:description" content="${description ?? siteDescription}">
    ${image ? `<meta property="og:image" content="${image.startsWith('/') ? `${baseUrl}${image}` : `${resolvedURL}${image}`}">` : ''}
    ...
    ${scripts ? scripts.map(script => `<script type="module" src="${script.startsWith('/') ? script : script}"></script>`).join('\n    ') : ''}
    ${styles ? styles.map(style => `<link rel="stylesheet" href="${style.startsWith('/') ? style : style}">`).join('\n    ') : ''}
    ${head || ''}
  `
