...
async function createTestUser (app, t) {
  ...
  // Register cleanup to run before app closes
  t.after(async () => {
    try {
      await app.pg.query(SQL`DELETE FROM users WHERE id = ${userId}`)
    } catch (err) {
...
describe('cleanup stale auth tokens', () => {
  ...
  it('does NOT delete protected tokens even if old', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('does NOT delete tokens newer than retention period', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('handles boundary case - token exactly 365 days old', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('handles empty result when no tokens to delete', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('handles no tokens at all', async (t) => {
    const app = await build(t)
  ...
  it('works with custom retention period - 30 days', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('works with custom retention period - 730 days (2 years)', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('validates retention period is positive integer', async (t) => {
    const app = await build(t)
    await createTestUser(app, t)
  ...
  it('actually deletes from database (verification test)', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('handles mixed scenario - old, protected, and recent tokens', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
  ...
  it('uses fallback for unsupported retention periods', async (t) => {
    const app = await build(t)
    const userId = await createTestUser(app, t)
