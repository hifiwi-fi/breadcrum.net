...
export function makeBookmarkP ({ fastify }) {
  ...
  async function bookmarkP (job) {
    ...
    let document
    if (resolveBookmark || resolveArchive) {
    ...
    if (resolveBookmark && document) {
      try {
        pageMetadata = await getSiteMetadata({ document })
      } catch (err) {
        log.warn(
          {
            error: err,
    ...
    if (resolveBookmark) {
      ...
      if (pageMetadata?.title && userProvidedMeta.title === url) {
    ...
    if (resolveArchive) {
      const article = await extractArchive({ document })
